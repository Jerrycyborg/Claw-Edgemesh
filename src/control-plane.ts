...
export function buildControlPlane(
  ...
  app.post<{ Params: { nodeId: string }; Body: HeartbeatRequest }>("/v1/nodes/:nodeId/heartbeat", async (req, reply) => {
  ...
  app.post<{ Body: Omit<Task, "status" | "createdAt" | "schemaVersion"> }>("/v1/tasks", async (req) => {
    const newTask: Task = {
      ...req.body,
  ...
  app.post<{ Params: { nodeId: string } }>("/v1/nodes/:nodeId/tasks/claim", async (req) => {
    ...
    if (task) {
      ctx.emit({ type: "task.claimed", at: Date.now(), nodeId: req.params.nodeId, taskId: task.taskId });
    }
    return { ok: true, task };
  });
  ...
  app.post<{ Params: { taskId: string } }>("/v1/tasks/:taskId/ack", async (req, reply) => {
    ...
    ctx.emit({ type: "task.running", at: Date.now(), taskId: req.params.taskId, nodeId: task.assignedNodeId });
    return { ok: true };
  });

  app.post<{ Params: { taskId: string }; Body: TaskResult }>("/v1/tasks/:taskId/result", async (req, reply) => {
